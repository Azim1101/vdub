...
| 5 | Voice + mux (Kokoro TTS) | model wired, stage next |
...
| | |
|---|---|
| Disk, if you download everything | **1.27 GB** |
| **Peak RAM** | **591 MB** — the largest single model |
...
| Kokoro-82M TTS (Hindi voices) | 87 MB | Voice | yes |
...
- **Kokoro is preset voices, not voice cloning.** It will not reproduce the
  original actor. Cloning needs Chatterbox (1.73 GB), which is impractical
  on-device.
